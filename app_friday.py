...
plants_care = pd.read_csv('scraping/plant_info_wk2.csv')
...
if uploaded_file is not None:
    ...
    local_model = load_model('model/josh_vgg_v2')
...
#----------------two pictures under  the plant Genus---------------
    ...
    path = ('plant_examples') # exchange once data is in virtual machine
    ...
    cnn_model = load_model('model/augmented_basic_cnn')
    ...
    if st.button(f'Yes, my plant looks like {pred1}!'):
        ...
        while f'{today}.csv' not in os.listdir('weather_data'):
            weather.main()
        weather = pd.read_csv(f'weather_data/{today}.csv')
